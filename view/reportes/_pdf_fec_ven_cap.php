...
class PDF extends FPDF
{
	...
	{                
                ...
                $this->Ln();
                $this->SetXY(9,8);
                $this->Write(0,'REPORTE DE VENCIMIENTO DE CAPACITACIONES');		
		...
		$this->Cell($this->widths[0], $h, 'ITEM', $border, 0, 'C',$fill);                
                $this->Cell($this->widths[1], $h, 'NOMBRES', $border, 0, 'C',$fill);
                $this->Cell($this->widths[2], $h, 'APELLIDOS', $border, 0, 'C',$fill);
                $this->Cell($this->widths[3], $h, 'FECHA DE VENC. CAP', $border, 0, 'C',$fill);
                $this->Ln();
...
//            }
            ...
            foreach($rowsi as $r)
            {
                ...
                $this->Cell($w[1], $h,$r['nombre'], $border, 0, 'C', $fill); 
                //$nh = $this->nLineaBreak($w[2], $h, utf8_decode($r['doc']), 'L');
                $this->Cell($w[2], $h, $r['apellidos'], $border, 0, 'C', $fill);                
