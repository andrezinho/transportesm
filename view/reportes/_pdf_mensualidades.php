...
class PDF extends FPDF
{
	...
	{                
                ...
                $this->Ln();
                $this->SetXY(9,8);
                $this->Write(0,'REPORTE DE MENSUALIDADES POR FECHA');		
	...
	function FancyTable($rowsi,$rows)
	{
            ...
            foreach($rowsi as $r)
            {
                ...
                $this->MultiCell($w[2], $h, $r['remitente'], $border, 'L', $fill);                
                $this->Cell($w[1], $h,$r['concepto'], $border, 0, 'L', $fill);                  
                
                $this->Cell($w[3], $h,$r['chofer'], $border, 0, 'L', $fill);
                $this->Cell($w[4], $h,$r['placa'], $border, 0, 'C', $fill);
                $this->Cell($w[5], $h,$this->ffecha($r['fecha']), $border, 0, 'C', $fill);
				$this->Cell($w[6], $h,$r['observacion'], $border, 0, 'L', $fill);
				$this->Cell($w[7], $h,number_format($r['total'],2), $border, 0, 'C', $fill);
